...
public class ActionAutoRunMatches implements IPluginActionDelegate {
	...
	public void performActionNotUI(Session session, PrintWriter logWriter) {
		if (Main.getTggRuleCollection().getRuleList().size() == 0) {
			System.out.println("Please open a TGG rule file first. No rules available");
			return;
		}
		MatchManager manager = null;
		...
		switch (Main.getTggRuleCollection().getType()) {
			...
			case BACKWARD:
				...
				break;
			case COEVOLUTION:
				System.out.println("Sorry, this feature is not available for co-evolution transformations. Feature not available");
				return;
			case INTEGRATION:
				manager = new IntegrationMatchManager(state, false);
				break;
		...
		while (true) {
			...
			if (Main.getTggRuleCollection().getType() == TransformationType.SYNCHRONIZATION) {
				...
				// vv.huong
				matches = manager.findMatches(api);
			if (matches.isEmpty())
				break;
			boolean success = false;
			for (Match match : matches) {
				// System.out.println(match.toString());
				success = match.run(state, logWriter);
				if (!success) {
					logWriter.println("Failed to run match");
				} else {
					i++;
					break;
				}
