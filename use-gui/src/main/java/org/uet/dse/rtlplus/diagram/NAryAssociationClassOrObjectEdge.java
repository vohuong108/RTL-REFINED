...
/**
 * Represents the dashed line between an association class or link object and
 * the diamond node of an n-ary association.
 * 
 * @author lhamann
 *
 */
public class NAryAssociationClassOrObjectEdge extends EdgeBase {
	 ...
	 * the associationclass / objectlink.
	 */
	 ...
	 */
	public NAryAssociationClassOrObjectEdge(DiamondNode diamondNode, NodeBase associationClassNode, DiagramView diagram,
	...
	private void initNodeEdge(DiamondNode node, NodeBase nodeEdgeNode) {
		...
		fNENode = new WayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++, WayPointType.ASSOC_CLASS, getName(),
				fOpt);
		fConNode = new WayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++, WayPointType.ASSOC_CLASS_CON, getName(),
				fOpt);
	...
	public void onDraw(Graphics2D g) {
		...
		try {
			DirectedEdgeFactory.drawDashedEdge(g, (int) fDiamondNode.getCenter().getX(),
					(int) fDiamondNode.getCenter().getY(), (int) fAssociationClassOrLinkObjectNode.getCenter().getX(),
	...
	/**
	 * Calculates the mid point of two given points which form a straight line. This
	 * calculated mid point marks the connection point, where the dashed line
	 * connectes to the solid line.
	 * 
	 * @param x1
	...
	private void updateConnectionPoint(double x1, double y1, double x2, double y2) {
		if ((fWayPoints.size() <= 3 && !fConNode.isSelected() && !fConNode.wasMoved() && !isReflexive())
				|| (fWayPoints.size() <= 6 && !fConNode.isSelected() && !fConNode.wasMoved() && isReflexive())) {
			Point2D conPoint = calcConnectionPoint(x1, y1, x2, y2);
			Point2D nep = fAssociationClassOrLinkObjectNode
					.getIntersectionCoordinate(fAssociationClassOrLinkObjectNode.getCenter(), conPoint);
