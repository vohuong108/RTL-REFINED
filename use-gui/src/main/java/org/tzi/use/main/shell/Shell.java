...
public final class Shell implements Runnable, PPCHandler {
	...
	public void run() {
		...
		for (String NAME : TESTCASE) {
			...
			System.out.println("START EXECUTE " + NAME.toUpperCase());
			...
			try
			{
				...
				for(int i = 5; i <= 5; i++) {
					...
					try {
						File file = new File(
								String.format("F:/Vo Huong/KLTN/LabWorkflow/testcase/%s_%d.txt", NAME, levelScale));
						BufferedReader br = new BufferedReader(new FileReader(file));

						String st;
						while ((st = br.readLine()) != null) {
							if(!st.trim().isEmpty()) {
								processLineSafely(st);
								Thread.yield();
								Log.resetOutputFlag();
							}
						}
					...
					ActionAutoRunMatches autoRunMatches = new ActionAutoRunMatches();
					autoRunMatches.performActionNotUI(fSession, writer);
				...
				try {
					if (writerMeasure != null) writerMeasure.close();
				}
				catch ( IOException e) {
				}
			}
