 ...
 * <ul>
 * <li>Enable integer type settings.</li>
 * <li>For each integer attribute whose maximum defined value is greater than 0
 * and whose classes maximum number of instances is not 0, set minimum and
 * maximum number of its owning class to 0.</li>
 * <li>For each integer attribute whose maximum defined value is greater than 0
 * and whose classes maximum number of instances is not 0, set minimum and
 * maximum number of defined attributes to 0.</li>
 * </ul>
 * 
 * @author Jan Prien
 *
 */
final class IntegerSettingsNessessaryValidator extends AbstractIntegerSettingsValidityRuleValidator {
	...
	protected ValidityRuleViolence[] getViolations(IntegerSettings config, IModel model) {
		...
		Map<ClassSettings, Set<AttributeSettings>> attributeSettingsWithMaxGreaterThanZeroForClassSettingsWithInstances = this
				.getClassSettingsWithInstancesAndIntegerAttributeWithMaxGreaterThanZero(
		...
		if (!config.isEnabled() && attributeSettingsWithMaxGreaterThanZeroForClassSettingsWithInstances.size() > 0) {
			...
			fixes.add(new SetIntegerSettingsEnabledFix(config.getSettingsConfiguration(), model,
					UIElements.Enable(TypeConstants.INTEGER), true, config, true));
			...
			return new ValidityRuleViolence[] {
					new ValidityRuleViolence(UIElements.TypeSettingsNessessary(TypeConstants.INTEGER),
