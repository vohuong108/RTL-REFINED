 ...
 * <ul>
 * <li>Disable integer type settings.</li>
 * </ul>
 * 
 * @author Jan Prien
 *
 */
final class IntegerSettingsUnnessessaryValidator extends AbstractIntegerSettingsValidityRuleValidator {
	...
	protected ValidityRuleViolence[] getViolations(IntegerSettings config, IModel model) {
		...
		if (config.isEnabled() && attributeSettingsWithMaxGreaterThanZeroForClassSettingsWithInstances.size() == 0) {
			...
			fixes.add(new SetIntegerSettingsEnabledFix(config.getSettingsConfiguration(), model,
					UIElements.Disable(TypeConstants.INTEGER), true, config, false));

			Set<ClassSettings> classSettingsWithoutInstancesAndIntegerAttribute = this
					.getClassSettingsWithoutInstancesAndIntegerAttribute(config.getSettingsConfiguration(), model);

			Map<ClassSettings, Set<AttributeSettings>> attributeSettingsWithMaxZeroForClassSettings = this
					.getClassSettingsWithIntegerAttributeWithMaxZero(config.getSettingsConfiguration(), model);
			...
			return new ValidityRuleViolence[] {
					new ValidityRuleViolence(UIElements.TypeSettingsUnnessessary(TypeConstants.INTEGER),
