 ...
 * <ul>
 * <li>Enable string type settings.</li>
 * <li>For each string attribute whose maximum defined value is greater than 0
 ...
 *
 */
final class StringSettingsNessessaryValidator extends AbstractStringSettingsValidityRuleValidator {
	...
	ValidityRuleViolence[] getViolations(StringSettings config, IModel model) {
		...
		if (!config.isEnabled() && attributeSettingsWithMaxGreaterThanZeroForClassSettingsWithInstances.size() > 0) {
			...
			fixes.add(new SetStringSettingsEnabledFix(config.getSettingsConfiguration(), model,
					UIElements.Enable(TypeConstants.STRING), true, config, true));
			...
			return new ValidityRuleViolence[] {
					new ValidityRuleViolence(UIElements.TypeSettingsNessessary(TypeConstants.STRING),
