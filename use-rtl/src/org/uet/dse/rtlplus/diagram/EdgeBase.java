...
import org.uet.dse.rtlplus.diagram.waypoints.AttachedWayPoint;
import org.uet.dse.rtlplus.diagram.waypoints.SourceWayPoint;
import org.uet.dse.rtlplus.diagram.waypoints.TargetWayPoint;
import org.uet.dse.rtlplus.diagram.waypoints.WayPoint;
import org.uet.dse.rtlplus.diagram.waypoints.WayPointComparator;
...
public abstract class EdgeBase extends DirectedEdgeBase<NodeBase> implements Selectable {
	...
	/**
	 * Initial counter for the IDs for the nodes which are laying on this edge.
	 */
	...
	/**
	 * Because of the qualifier nodes, calculation has do be done twice the first
	 * time. This attribute handles this.
	 */
	 ...
	 */
	public EdgeBase(NodeBase source, NodeBase target, String edgeName, DiagramView diagram) {
		super(source, target);
		...
		fSourceWayPoint = new SourceWayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++, fEdgeName, fOpt);

		fTargetWayPoint = new TargetWayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++, fEdgeName, fOpt);

		fWayPoints.add(fSourceWayPoint);
		fWayPoints.add(fTargetWayPoint);

		// When source and target are moved, we also want to move the user defined way
		// points
	...
	/**
	 * An edge can return a value here to give the drawing engine a hint for the
	 * minimum height of the source node
	 * 
	...
	/**
	 * An edge can return a value here to give the drawing engine a hint for the
	 * minimum height of the target node
	 * 
	 * @return a hint for the height or 0
	 */
	...
	/**
	 * Gets the source node the first edge part is drawn from. Can be another source
	 * the the semantic source node, e.g., a qualifier node.
	 * 
	 * @return
	 */
	...
	/**
	 * True if a special drawing source is attached to this edge. When true
	 * {@link #getDrawingSource()} differs from {@link #fSource}
	 * 
	 * @return
	 */
	...
	/**
	 * True if a special drawing target is attached to this edge. When true
	 * {@link #getDrawingTarget()} differs from {@link #fTarget}
	 * 
	 * @return
	 */
	...
	/**
	 * Sets an offset value for the vertical placement of a special source. It is
	 * left to concrete implementations how to handle the value 0. For example it
	 * could mean to center the special source.
	 */
	...
	/**
	 * Gets the offset value for the vertical placement of a special source. It is
	 * left to concrete implementations how to handle the value 0. For example it
	 * could mean to center the special source.
	 */
	...
	/**
	 * Sets an offset value for the vertical placement of a special target. It is
	 * left to concrete implementations how to handle the value 0. For example it
	 * could mean to center the special source.
	 */
	...
	/**
	 * Draws the edge to the given Graphics object. When first drawn
	 * {@link #onFirstDraw(java.awt.Graphics2D)} is called and afterwards
	 * {@link #onDraw(java.awt.Graphics2D)}. After the first draw only
	 * {@link #onDraw(java.awt.Graphics2D)} is called.
	 * 
	 * @param g
	...
	/**
	 * Called once when the edge is drawn the first time. Can be used for example to
	...
	/**
	 * Is beneath the x,y position an edge, than an additional node will be added on
	 * this edge.
	 *
	 * @param x
	 *            x position to check
	...
	public WayPoint occupiesThanAdd(int x, int y, int clickCount) {
		...
		while (it.hasNext()) {
			...
			if (occupies && clickCount == 2) {
				...
				if (!n2.dimension().contains(x, y) || !n1.dimension().contains(x, y)) {

					WayPoint newNode = new WayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++, WayPointType.USER,
							fEdgeName, fOpt);
	 ...
	 *            Node to be added.
	 * @param n1
	 *            Behind this node <code>node</code> will be added to the list of
	 *            nodes.
	 */
	...
	/**
	 * Resets the ids of all nodes on this edge. The nodes have allways an
	 * increasing ID starting at the source node.
	 */
	void reIDNodes() {
	...
	/**
	 * Calculates the space between the lines if there are more than one edge
	 * between two nodes.
	 */
	 ...
	 * {@link #calculateNewPositions(java.util.Set)} this can be used by an edge to
	 * calculate positions of additional related elements. Currently used for
	 * <code>QualierNode</code>s.
	 */
	...
	/**
	 * Calculates the position of a single edge (from center to center) or multiple
	 * edges (with space between edges) between two nodes. Primitive operation (see
	 * TemplateMethod in GOF) for template method
	 * {@link #calculateNewPositions(java.util.Set)}.
	 * 
	 * @param edges
	 *            All edges between to given nodes.
	 */
	protected void doCalculateNewPositions(Set<EdgeBase> edges) {
		...
		if (edges.size() == 1) {
			...
			if (intersection.isLocatedNorth() || intersection.isLocatedSouth()) {
				...
				projection = (Math.max(sourceRec.getWidth(), targetRec.getWidth()) - space * (edges.size() - 1)) / 2.0;
				...
				for (EdgeBase e : edges) {
					...
					aligned.sourceWayPoint.updatePosition((drawingSourceEqualsSource ? sDeltaX : 0), 0,
							aligned.sourceWayPointNext);
					aligned.targetWayPoint.updatePosition((drawingTargetEqualsTarget ? tDeltaX : 0), 0,
							aligned.targetWayPointNext);
				...
				// calculates the length of the space to the first edge
				// for the bigger rectangle
				projection = (Math.max(sourceRec.getHeight(), targetRec.getHeight()) - space * (edges.size() - 1))
						/ 2.0;
				...
				for (EdgeBase e : edges) {
					...
					aligned.sourceWayPoint.updatePosition(0, (drawingSourceEqualsSource ? sDeltaY : 0),
							aligned.sourceWayPointNext);
					aligned.targetWayPoint.updatePosition(0, (drawingTargetEqualsTarget ? tDeltaY : 0),
							aligned.targetWayPointNext);
	...
	/**
	 * Saves information about the aligned objects of another edge, i.e., the
	 * correct source objects when two edges between two nodes save source and
	...
	/**
	 * Returns the informations of the other edge aligned to this edge, e.g., the
	 * returned source is equal to the source of this edge.
	 * 
	 * @param e
	 *            The <code>EdgeBase</code> to be aligned
	 * @return
	 */
	...
	/**
	 * Determines the <code>WayPoint</code> of this edge which is located most to
	 * the direction specified by <code>direction</code>.
	 * 
	 * @return The <code>WayPoint</code> which is located most to the specified
	 *         direction.
	 */
	public WayPoint getWayPointMostTo(Direction direction) {
		WayPoint result = fWayPoints.get(0);
		for (WayPoint wp : fWayPoints) {
			if (Direction.getDirection(result.getCenter(), wp.getCenter()).isLocatedTo(direction))
				result = wp;
		}
	...
	/**
	 * If not null and not an empty string, the attribute <code>kind</code> with the
	 * returned value will be written into the store info
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param hidden
	 *            If this edge should be hidden or not.
	...
	protected void restoreWayPoint(PersistHelper helper, Element wayPointElement, int version) {
		...
		int specialId = Integer.valueOf(helper.getElementStringValue(wayPointElement, LayoutTags.SPECIALID));
	...
	public final void restorePlacementInfo(final PersistHelper helper, Element elementNode, int version) {
		...
		NodeList wayPoints = (NodeList) helper.evaluateXPathSave(elementNode,
				"./edgeproperty[@type='NodeOnEdge' or @type='WayPoint']", XPathConstants.NODESET);
		...
		NodeList properties = (NodeList) helper.evaluateXPathSave(elementNode,
				"./edgeproperty[not (@type='NodeOnEdge' or @type='WayPoint')]", XPathConstants.NODESET);
