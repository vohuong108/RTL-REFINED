 ...
 * PseudoNode in case of an association class. It only provides semantic
 * informations about the target association end.
 * 
...
public class AssociationOrLinkPartEdge extends EdgeBase {
	 ...
	 */
	public AssociationOrLinkPartEdge(NodeBase source, NodeBase target, MAssociationEnd targetEnd, DiagramView diagram,
		...
		fAssocName = new AssociationName(fAssoc.name(), source, target, fSourceWayPoint, fTargetWayPoint, fOpt, this,
				assoc, isLink);

		fTargetEnd = targetEnd;

		fTargetRolename = new Rolename(targetEnd, target, source, fTargetWayPoint, fSourceWayPoint, fOpt,
				Rolename.TARGET_SIDE, this);

		fTargetMultiplicity = new Multiplicity(targetEnd, target, source, this, fTargetWayPoint, fSourceWayPoint, fOpt,
	...
	public PlaceableNode getWayPoint(double x, double y) {
		...
		if (fOpt.isShowRolenames() && getTargetRolename() != null && getTargetRolename().occupies(x, y)) {
		...
		} else if (fOpt.isShowMutliplicities() && getTargetMultiplicity() != null
				&& getTargetMultiplicity().occupies(x, y)) {
			res = getTargetMultiplicity();
		} else if (fOpt.isShowAssocNames() && getAssocName() != null && getAssocName().occupies(x, y)) {
			res = getAssocName();
	...
	/**
	 * Draws the last line segment, as an association, composition, aggregation,
	 * directed edge etc.
	 */
