...
/**
 * Extends the {@link org.uet.dse.rtlplus.diagram.BinaryAssociationOrLinkEdge}
 * to additionally draw the line between the rectangle of the association class/
 * objectlink and the corresponding classes/objects.
 *
...
public class BinaryAssociationClassOrObject extends BinaryAssociationOrLinkEdge {
	...
	/**
	 * Point which connects the dashed line of an association class / object link to
	 * the solid line.
	 */
	WayPoint associationConnectionPoint;

	/**
	 * The first way point the connection point is related to. Without user defined
	 * way points this is the source way point.
	 */
	WayPoint connectionPointRefWayPoint1;

	/**
	 * The second way point the connection point is related to. Without user defined
	 * way points this is the target way point.
	 */
	WayPoint connectionPointRefWayPoint2;

	/**
	 * The NodeBase of instance ClassNode or ObjectNode displaying the class part of
	 * the association class / object link.
	 */
	 ...
	 */
	public BinaryAssociationClassOrObject(NodeBase source, NodeBase target, MAssociationEnd sourceEnd,
			MAssociationEnd targetEnd, NodeBase associationClassOrObjectNode, DiagramView diagram, MAssociation assoc,
			int groupId) {
	 ...
	 */
	public BinaryAssociationClassOrObject(NodeBase source, NodeBase target, MAssociationEnd sourceEnd,
			MAssociationEnd targetEnd, NodeBase associationClassOrObjectNode, DiagramView diagram, MLink link,
			int groupId) {
	...
	private void initNodeEdge(NodeBase associationClassOrObjectNode) {
		...
		fAssociationClassOrObjectWayPoint = new WayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++,
				WayPointType.ASSOC_CLASS, getName(), fOpt);

		associationConnectionPoint = new WayPoint(fSource, fTarget, this, fNodesOnEdgeCounter++,
				WayPointType.ASSOC_CLASS_CON, getName(), fOpt);
	 ...
	 * 
	 * @param g
	 *            The graphics object the dashed line is drawn into.
	 */
	private void drawNodeEdge(Graphics2D g) {
		...
		try {
			DirectedEdgeFactory.drawDashedEdge(g, (int) associationConnectionPoint.getCenter().getX(),
	...
	/**
	 * Calculates the new position of the connection point where the dashed line
	 * connects to the solid line.
	 */
	private void updateConnectionPoint() {
		...
		Point2D nep = fAssociationClassOrObjectNode.getIntersectionCoordinate(fAssociationClassOrObjectNode.getCenter(),
				conPoint);
	 ...
	 * 
	 * @see
	 * org.tzi.use.gui.views.diagrams.EdgeBase#removeNodeOnEdge(org.tzi.use.gui.
	 * views.diagrams.waypoints.WayPoint)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.tzi.use.gui.views.diagrams.BinaryAssociationOrLinkEdge#
	 * restoreAdditionalInfo(org.tzi.use.gui.util.PersistHelper,
