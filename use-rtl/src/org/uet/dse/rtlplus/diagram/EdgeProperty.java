...
public abstract class EdgeProperty extends PlaceableNode {
	...
	/**
	 * This value is calculated when the property is drawn the first time and
	 * represents the text bounds of the caption.
	 */
	...
	public EdgeProperty(NodeBase source, WayPoint sourceWayPoint, NodeBase target, WayPoint targetWayPoint,
			boolean isLink) {
		this.fSource = source;
	...
	/**
	 * Returns the bounds of the drawn text of {@link #name()}. The value is set
	 * when drawing the first time.
	 * 
	 * @return
	 */
	...
	/**
	 * Calculates the default position, which is used when not placed by the user.
	 */
	...
	/**
	 * Cached instance of the text layout for this property. Is lazy initialized at
	 * first call to {@link #getTextLayout(java.awt.Graphics2D)}
	 */
	protected TextLayout textLayout = null;

	/**
	 * Used to enable an improved positioning when loading layouts saved in version
	 * 1
	 */
	 ...
	 * 
	 * @param g
	 *            The GraphicContext to draw with
	 * @return
	 */
	...
	/**
	 * Draws a stroked line around the bounds of this edge property and lines to the
	 * attached source or target way point. If not connected to a specific way
	 * point, i.e., <code>{@link #fSide} == 0</code>, lines to both sides are drawn.
	 * 
	 * @param g
	 */
	protected void drawSelected(Graphics2D g) {
		...
		g.drawLine((int) attachedPosition.getX(), (int) attachedPosition.getY(), (int) sourcePoint.getX(),
	...
	protected void drawTextCentered(Graphics2D g) {
		...
		float y = Math.round(
				(float) (bounds.getCenterY() + (layout.getAscent() + layout.getDescent()) / 2 - layout.getDescent()));
	...
	protected void restoreAdditionalInfo(PersistHelper helper, Element nodeElement, int version) {
		...
		if (version == 1) {
			...
			if (isUserDefined) {
				...
				y = Math.floor(
						y - (layout.getAscent() + layout.getDescent()) / 2 - layout.getDescent() - MARGIN_VERTICAL / 2);
			}
		} else {
